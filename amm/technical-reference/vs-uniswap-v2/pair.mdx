---
title: "SoroswapPair Comparison"
description: "Function by function comparison of SoroswapPair with the UniswapV2Pair contract, and what differs."
---
...
The sections below compare that early Soroswap pair contract with the UniswapV2 pair contract, using the latter as a reference point. This lets us discuss why certain features were implemented or left out at the time. For the contract as it stands today, see [Contracts](/amm/technical-reference/contracts).
...
Soroswap adopted a similar approach, implementing this functionality in the `get_reserves` function. Note that the SoroswapPair contract as deployed today returns exactly two values from `get_reserves`, the reserve of `token_0` and the reserve of `token_1`. No block timestamp is returned. The snippet below is from the early version.
...
That early version also carried a `get_block_timestamp_last` function, which returned the timestamp of the last block, defaulting to 0 if it did not exist.
...
* At the time of writing, the objective was simply to implement the UniswapV2 Pair and Factory contracts, so the design was:
...
That changed in a later iteration. The Periphery contracts were implemented (see [https://github.com/Uniswap/v2-periphery](https://github.com/Uniswap/v2-periphery) for the Uniswap equivalent), SoroswapRouter shipped and is deployed on mainnet, and `deposit` now takes only the `to` address and requires the tokens to be sent to the pair before it is called.
...
The equivalent function in Soroswap at the time was as follows. The comment inside the snippet anticipated the Router: SoroswapRouter was later written and deployed on mainnet, and the amount calculation and routing logic moved there. The pair `swap` function now takes `amount_0_out`, `amount_1_out` and `to`, matching the UniswapV2 shape shown above.
...
The equivalent function in Soroswap at the time was as follows. The comments inside the snippet point at work that has since been done: SoroswapRouter shipped, the minimum-amount checks live there, and the pair `withdraw` function now takes only the `to` address and returns the two withdrawn amounts.
