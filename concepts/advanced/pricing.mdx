---
title: "Pricing"
description: "How pool reserves determine swap prices, and how to price exact input and exact output trades safely."
---

## How are prices determined?

As we learned in [Protocol Overview](/amm), each pair on Uniswap is actually underpinned by a liquidity pool. Liquidity pools are smart contracts that hold balances of two unique tokens and enforces rules around depositing and withdrawing them. The primary rule is the [constant product formula](/amm/glossary#x--y--k). When a token is withdrawn (bought), a proportional amount must be deposited (sold) to maintain the constant. The ratio of tokens in the pool, in combination with the constant product formula, ultimately determine the price that a swap executes at.
...
At a high level, in Uniswap V2, _trades must be priced in the periphery_. The good news is that the [library](/amm/technical-reference/contracts/soroswap-library) provides a variety of functions designed to make this quite simple, and all swapping functions in the [router](/amm/technical-reference/contracts/soroswap-router) are designed with this in mind.
...
There are, of course, other options for oracles, including [native V2 oracles](/concepts/oracles).

### Exact Input

If you'd like to send an exact amount of input tokens in exchange for as many output tokens as possible, you'll want to use [getAmountsOut](/amm/technical-reference/contracts/soroswap-router). The equivalent SDK function is [getOutputAmount](/amm/technical-reference/contracts/soroswap-pair), or [minimumAmountOut](/amm/technical-reference/contracts/soroswap-pair) for slippage calculations.

### Exact Output

If you'd like to receive an exact amount of output tokens for as few input tokens as possible, you'll want to use [getAmountsIn](/amm/technical-reference/contracts/soroswap-router). The equivalent SDK function is [getInputAmount](/amm/technical-reference/contracts/soroswap-pair), or [maximumAmountIn](/amm/technical-reference/contracts/soroswap-pair) for slippage calculations.
